...
When you think of a **wallet address**, you probably think of a string of characters (or in Rafiki's case, a neat URL) that acts as a source or destination for payments. For example:

`https://ilp.interledger.cards/007`

Traditionally, when a service like Rafiki queries that address, it receives structured JSON:
...
With **Smart Redirect** enabled, Rafiki detects when a request is coming from a
browser (`Accept: text/html`). Instead of showing raw JSON, the request automatically
redirects to a user-friendly payment page defined in your environment settings.
