...
This year, a humble part of Interledger Protocol (ILP for short) history took center stage as Stefan Thomas [gave a keynote](https://www.youtube.com/watch?v=Djw6wMXFv1Q&t=3240s) on how years ago, the Interledger Protocol became more fast & reliable by introducing packetization to the protocol.
...
![Two-Phase Transfer Diagram](/developers/img/blog/2025-01-31/2p-transfer.png)

In the first phase, the sender creates packets known as [Prepare packets](/developers/rfcs/interledger-protocol/#ilp-prepare), which each contain some fraction of the total send amount configured by each peer along the ILP network path, the condition generated using SPSP, the final destination of the packet in the network, and an expiration time for the packet.
...
![ILP Prepare Packet Specification](/developers/img/blog/2025-01-31/ilp-prepare-spec.png)
...
If a connector on the packet's path to the receiver determines that the connector before it doesn't have enough balance to fulfill the packet, it will send a [Reject packet](/developers/rfcs/interledger-protocol/#ilp-reject) back along that path to the receiver.
...
![ILP Reject Packet Specification](/developers/img/blog/2025-01-31/ilp-reject-spec.png)
...
The receiver fulfills a Prepare packet by sending a [Fulfill packet](/developers/rfcs/interledger-protocol/#ilp-fulfill) back to the sender with the preimage from which the Prepare packet's condition was generated.

![ILP Fulfill Packet Specification](/developers/img/blog/2025-01-31/ilp-fulfill-spec.png)
...
- [Peering Connectors](/developers/rfcs/peering-clearing-settling)
